...
class BlobsDataModule(LightningDataModule): # Data module for Two Moons dataset
    def __init__(self, batch_size=32, centers = 10, train_transforms=None, test_transforms=None):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        self.val_dataset = CustomTensorDataset((torch.from_numpy(self.val_data).float(), torch.from_numpy(self.val_labels), torch.from_numpy(blobs_coarse_labels[self.val_labels])),transform = self.test_transforms)
