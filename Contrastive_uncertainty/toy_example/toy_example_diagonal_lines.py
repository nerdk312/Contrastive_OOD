...
"""==================================================="""
ppline     = 100 # Num samples to generate (points per linen)
n_lines    = 4
noise_perc = 0.15
...
cls   = [x*np.ones(int(ppline*noise_perc)) for x in range(n_lines)] # Classes labels for each of the data points in lines
train_lines = np.concatenate(lines, axis=1).T
train_cls   = np.concatenate(cls) # class labels

# Make points for the test lines
...
###############
# Networks -regularised and normal
base_net     = Backbone()
...
def train(net2train, p_switch=0):
    ...
    for i in range(train_iter):
        ...
        embed        = net2train(batch) # Obtain embeddings

        unique_cls   = np.unique(train_labels) # labels
        indices      = np.arange(len(batch)) # obtain a list from 1 to batch length
        class_dict   = {i:indices[train_labels==i] for i in unique_cls} # group together the indices belonging to a certain class

        # Process of choosing the triplets for the triplet
        ...
        # anchors, positives and negatives for the triplet loss
        ...
        if p_switch>0:
            # Randomly switching negative samples and positive samples, which pushes samples of the same class apart
...
###############
# Loss value for base model and regularised model
...
def get_embeds(net):
    _ = net.eval()
    # Obtain training embeddings as well as testing embeddings
    with torch.no_grad():
    ...
    # calculate singular values of train embeddings and then normalise
