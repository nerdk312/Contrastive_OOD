...
# Override the current code to look at using multiple labels for the task    
class OfflineLabelMemory(nn.Module):
    ...
    def __init__(self, length, feat_dim, memory_momentum, num_cluster,
                 **kwargs):
        ...
        # Make label bank, which is multidimensional to correspond to multiple values
        self.label_bank = torch.zeros((length,len(num_cluster)), dtype=torch.long, device='cuda')
