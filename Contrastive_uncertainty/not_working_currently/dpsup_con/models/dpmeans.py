...
from Contrastive_uncertainty.dpsup_con.models.clustering import Clustering
...
class DPMeans(object):
    ...
    def cutoff(self):
        """Distance cutoff. """
        return self._cutoff
    # Set value for distance threshold
    @cutoff.setter
    ...
    # Assign data point to a cluster centre
    def assign_point(self, index):
        ...
        # Nawid -Calculates the distance of a data point
        point = self.data[index]
        dist = np.sum((point - self.centers)**2, 1)
        # Nawid - assigns to cluster if lower than threshold
        ...
        if k >= len(self._centers):
            self._add_new_batch()
        # Nawid - set datapoint as new cluster
