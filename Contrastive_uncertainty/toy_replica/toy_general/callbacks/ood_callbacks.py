...
class Aggregated_Mahalanobis_OOD(pl.Callback):
    ...
    def forward_callback(self,trainer,pl_module):
        ...
        test_loader = self.Datamodule.test_dataloader()
        ood_loader = self.OOD_Datamodule.test_dataloader()
        assert len(self.vector_level) == len(self.label_level)
        features_train = []
        labels_train = []
        features_test = []
        features_ood = []

        for index in len(self.vector_level):
            train_features, train_labels = self.get_features(pl_module, train_loader,self.vector_level[index], self.label_level[index])
            test_features, test_labels = self.get_features(pl_module, test_loader,self.vector_level[index], self.label_level[index]) 
            ood_features, ood_labels = self.get_features(pl_module, ood_loader, self.vector_level[index], self.label_level[index])    
            
            features_train.append(train_features)
            features_test.append(test_features)
            features_ood.append(ood_features)
    ...
    def get_scores_multi_cluster(self,ftrain, ftest, food, ypred):
        ...
        total_din = []
        total_dood = []
        for index in range(len(self.vector_level)):
            ...
            # Need to have a mapping for the different results to see how the performance changes for the task
    ...
    def get_eval_results(self,ftrain, ftest, food, labelstrain):
        ...
        wandb.log({self.log_name + f' Full Aggregated AUROC: {self.OOD_dataname}': auroc})
    ...
    def log_confidence_scores(self,Dtest,DOOD):  
        ...
        true_histogram_name = self.true_histogram + f': Full Aggregated'
        ood_histogram_name = self.ood_histogram + f': Full Aggregated: {self.OOD_dataname}'
