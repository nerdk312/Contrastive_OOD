...
class One_Dim_Mahalanobis_Similarity(pl.Callback):
    ...
    def get_scores(self, ftrain, ftest, labelstrain, labelstest):
        ...
        for class_i in np.unique(labelstrain):
            ...
            for class_j in np.unique(labelstest):
                ...
                KL = [kl_divergence(dtrain[class_i][:,dim],dtest[class_j][:,dim]) for dim in range(val)] #range(len(class_eigvals))]
                class_KL.append(KL)
        ...
        subprocess.call([
            'ffmpeg', '-framerate', '2', '-i', 'Images/file%02d.png', '-r', '30', '-pix_fmt', 'yuv420p',
            video_filename
        ])
    ...
    def get_eval_results(self, ftrain, ftest, labelstrain,labelstest):
        ...
        self.get_scores(ftrain_norm, ftest_norm, labelstrain, labelstest)    
