...
class Visualisation(pl.Callback): # General class for visualisation
    ...
    def compute_representations(self, pl_module, loader):
        features = []
        collated_labels = []
        #Make it so that only the fine grained labels are obtained which can be used for the datamodule as well as OOD datamodule
        for i, (images, labels,*_) in enumerate(tqdm(loader)): # Obtain data and labels from dataloader
